...
class PetContainer(BaseGameDataContainer):
    ...
    def get_effect_bonuses(self):
        if not self.user_data:
            log.debug("No user_data available")
            return

        economy_model = self.user_data.get('pEconomyModel', {})
        if not economy_model:
            log.warning("Missing 'pEconomyModel' in user_data")
            return

        pet_effects = economy_model.get('PetEffects', [])
        for bonus in pet_effects:
