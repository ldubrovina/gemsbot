...
from configurations import CONFIG, TOKEN
...
from search import TeamExpander
...
class DiscordBot(BaseBot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.views = Views(self.my_emojis)  # Передаем текущий словарь эмодзи
        self.pet_rescues = []
    ...
    async def on_ready(self):
        log.debug("on_ready event triggered.")
        ...
        log.debug(f"Invite URL generated: {self.invite_url}")
        ...
        log.debug("Bot presence updated.")
        ...
        log.debug(f'Loaded {len(self.pet_rescues)} pet rescues after restart.')        # Проверка и регистрация слэш-команд только при необходимости
        guild_id = CONFIG.get('slash_command_guild_id')
        try:
            existing_commands = await get_all_commands(self.user.id, TOKEN, guild_id=guild_id)
            log.debug(f'Found {len(existing_commands)} existing slash commands')

            # Сравниваем существующие команды с текущими
            commands_to_register = []
            for command in COMMAND_REGISTRY:
                if 'description' not in command:
                    continue

                existing_command = next((c for c in existing_commands if c['name'] == command['function']), None)
                if not existing_command or self.command_changed(existing_command):
                    commands_to_register.append(command)

            # Регистрируем команды только если есть изменения
            if commands_to_register:
                log.info(f'Registering {len(commands_to_register)} new or modified slash commands...')
                for command in commands_to_register:
                    try:
                        await add_slash_command(
                            self.user.id,
                            TOKEN,
                            guild_id,
                        ...
                        log.debug(f'Successfully registered slash command {command["function"]}')
                    except Exception as e:
                        log.error(f'Failed to register command {command["function"]}: {e}')
            else:
                log.info('All slash commands are up to date, no registration needed')
        except Exception as e:
            log.error(f'Failed to check slash commands: {e}')

    async def on_interaction(self, interaction):
        """Обрабатывает взаимодействия со слэш-командами."""
        log.debug("Handling interaction...")
        log.debug(f"Interaction type: {interaction.type}")
        log.debug(f"Interaction data: {interaction.data}")
        ...
        if interaction.type == discord.InteractionType.application_command:
            ...
            log.debug(f"Command options: {options}")

            # Создаем объект FakeMessage для обратной совместимости
            message = FakeMessage(
                interaction.user,
                interaction.guild,
                interaction.channel,
                f"/{command_name} {' '.join(f'{k}={v}' for k,v in options.items())}"
            )
            message.interaction = interaction  # Добавляем объект interaction для доступа к response
            ...
            try:
                if command_name == "ping":
                    ...
                    embed = discord.Embed(
                        title="О боте",
                        description=f"{self.BOT_NAME} v{self.VERSION}\nРазработан для помощи в игре Gems of War.",
                        color=discord.Color.blue()
                    )
                    await interaction.response.send_message(embed=embed)

                elif command_name == "team_code":
                    log.debug("Executing 'team_code' command.")

                    await self.team_code(
                        message=message,
                        lang=options.get("lang", "ru"),
                        team_code=options.get("team_code"),
                        shortened=options.get("shortened", False),
                        lengthened=options.get("lengthened", False),
                        title=options.get("title", "")
                    )
            except Exception as e:
                log.error(f"Error in command {command_name}: {str(e)}")
                try:
    ...
    async def get_function_for_command(self, user_command, user_prefix):
        log.debug(f"Checking command: {user_command} with prefix: {user_prefix}")
        for command in COMMAND_REGISTRY:
            log.debug(f"Testing pattern: {command['pattern']} for command: {command['function']}")
            match = command['pattern'].search(user_command)
            if not match:
                log.debug(f"No match for pattern: {command['pattern']}")
                continue
            groups = match.groupdict()
            log.debug(f"Match found. Groups: {groups}")

            if groups.get('prefix', user_prefix) == user_prefix:
                log.debug(f"Prefix matches. Returning function: {command['function']}")
                return getattr(self, command['function'], None), groups
    ...
    async def render_campaign_lines(self, message, campaign_data, task_skip_costs, lang):
        task_skip_costs = task_skip_costs or {}  # Убедимся, что task_skip_costs определён
        for category, tasks in campaign_data.items():
            category = category or ""  # Убедимся, что category определён
            category_lines = [f'**{task["title"]}**: {task["name"].replace("-->", "→")}' for task in tasks]
            category_lines = category_lines or []  # Убедимся, что category_lines определён
            color = CAMPAIGN_COLORS.get(category, self.WHITE)
            color = color or self.WHITE  # Убедимся, что color определён
    ...
    async def on_message(self, message):
        if message.author.bot:
            log.debug("Message ignored: sent by a bot.")
            return

        log.debug("on_message event triggered.")
        ...
        log.debug(f"Detected prefix: {my_prefix}")
        func, params = await self.get_function_for_command(user_command, my_prefix)
        if not func:
            log.debug("No matching command found.")
            return

        log.debug(f"Executing command function: {func.__name__}")
        await self.refresh_emojis()
        ...
        if await self.special_needed(message):
            log.debug("Special permissions required, command execution aborted.")
            return
    ...
    async def team_code(self, message, lang, team_code=None, shortened=False, lengthened=False, **kwargs):
        ...
        is_interaction = hasattr(message, 'interaction') and message.interaction is not None
        interaction_deferred = False

        try:
            # Немедленно отправляем defer для slash команд
            if is_interaction:
                try:
                    await message.interaction.response.defer(thinking=True)
                    interaction_deferred = True
                    log.debug("Interaction deferred")
                except Exception as defer_error:
                    log.error(f"Failed to defer interaction: {defer_error}")
            ...
            raw_team_code = team_code.strip()
            if raw_team_code.startswith('+'):
                team_code = raw_team_code[1:]
                lengthened = True
            elif raw_team_code.startswith('-'):
                team_code = raw_team_code[1:]
                shortened = True
            elif raw_team_code.startswith('[') and raw_team_code.endswith(']'):
                team_code = raw_team_code[1:-1]
            else:
                team_code = raw_team_code

            log.debug(f"Processing team code: {team_code}")
            ...
            log.error(f"Error in team_code processing: {e}")
            try:
    ...
    @guild_required
    @admin_required
    async def news_subscribe(self, message, platform=None, **__):
        if not platform:
            platform = CONFIG.get('default_news_platform')
            if not platform:
                log.error("Platform is not defined and no default is set in CONFIG.")
                await self.answer(message, "Ошибка: Платформа не указана и отсутствует значение по умолчанию.")
                return
    ...
    def command_changed(cls, command):
        ...
        if diff := cls.options_changed(command.get('options'), my_command.get('options')):
            ...
            return True
        return False    # Метод удален для отключения регистрации слэш-команд
    ...
    task_update_game_data = bot_tasks.task_update_game_data
    async def setup_hook(self):
        ...
        self.task_update_game_data.start()
...
if __name__ == '__main__':
    ...
    log.debug(f"Intents configuration: {intents}")

    client = DiscordBot(intents=intents, command_prefix=CONFIG.get('default_prefix'), help_command=None)

    if TOKEN is not None:
        log.debug("Starting bot with provided token.")
        client.run(TOKEN)
    else:
