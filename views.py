...
class Views:
    ...
    def __init__(self, emojis):
        ...
        self.jinja_env.filters['emoji'] = self.emoji_filter
        self.jinja_env.filters['banner_colors'] = self.banner_colors

    def emoji_filter(self, name):
        """Фильтр Jinja для получения эмодзи по имени."""
        return self.get_emoji(name)

    def get_emoji(self, name):
        """Получает эмодзи по имени."""
        safe_name = name.replace(' ', '_').lower()
        emoji = self.my_emojis.get(safe_name)
        if not emoji:
            return ""  # Возвращаем пустую строку вместо текстового представления
        return emoji  # Возвращаем объект эмодзи

    def get_emoji_as_image(self, name):
        """Получает эмодзи как встроенное изображение для Discord."""
        return self.get_emoji(name)  # Возвращаем объект эмодзи

    def banner_colors(self, banner):
        """Преобразует цвета баннера в эмодзи."""
        colors = []
        for color_code, value in banner['colors']:
            emoji = self.get_emoji_as_image(color_code)
            sign = '+' if value > 0 else ''
            colors.append(f'{emoji} {sign}{value}')
        return colors

    def render_embed(self, embed, template_name, **kwargs):
        def get_emoji(name, _=None):
            cut_name = name[:32]
            return self.get_emoji(cut_name)
        ...
        splits = content.split('<T>')
        # Устанавливаем description только если он еще не установлен
        if not embed.description and splits[0].strip():
            embed.description = splits[0]
    ...
    def render_team(self, team, author, shortened, lengthened, title=None):
        """Отображает информацию о команде с эмодзи."""
        e = discord.Embed(color=self.WHITE)
        # Устанавливаем изображение баннера, только если показываем детальную информацию
        if (shortened or lengthened) and team.get('banner'):
            thumbnail_url = f'{CONFIG.get("graphics_url")}/Banners/Banners_{team["banner"]["filename"]}_full.png'
            e.set_thumbnail(url=thumbnail_url)
          # Добавляем код команды как отдельное поле в эмбеде
        code = team.get("team_code", team.get("code", "???"))
        e.add_field(name="Код команды:", value=f"```\n[{code.strip('\"')}]\n```", inline=False)
        team['code'] = code  # Сохраняем оригинальный код для использования в шаблоне

        # Всегда добавляем эмодзи в данные команды
        for troop in team['troops']:
            troop['color_emoji'] = self.get_emoji_as_image(troop['color_code'])
            if 'affixes' in troop:
                troop['weapon_emoji'] = self.get_emoji_as_image('weapon')
            if 'raw_rarity' in troop:
                troop['rarity_emoji'] = self.get_emoji_as_image(troop['raw_rarity'].lower())
            # Обрезаем описание заклинания, если оно слишком длинное и нужно его показывать
            if lengthened and len(troop['spell']['description']) > 180:
                troop['spell']['description'] = troop['spell']['description'][:177] + '...'

        # Выбираем шаблон в зависимости от формата
        if shortened:
            e.title = f"{author}'s team (краткий формат)"
            template_file = 'team_shortened.jinja'
        elif lengthened:
            e.title = f"{author}'s team (расширенный формат)"
            template_file = 'team_lengthened.jinja'
        else:
            e.title = f"{author}'s team"
            template_file = 'team_info.jinja'
